...
internal sealed class OpenMeteoGeocodingProvider : IGeocodingProvider
{
    ...
    public async Task<Result<Location>> ResolveAsync(string city, string language, CancellationToken cancellationToken)
    {
        ...
        var openMeteoGeoResponse =
            await _geocodingClient.GeocodeAsync(city, CityCount, language, cancellationToken);
